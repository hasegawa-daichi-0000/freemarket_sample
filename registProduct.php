...
// パラメータ改ざんチェック
// ===============================
// GETパラメータはあるが、改ざんされている（URLをいじくった）場合、正しい商品データが取れていないのでマイページへ遷移させる
...
if (!empty($_POST)) {
  ...
  if (empty($dbFormData)) {
    ...
    if ($dbFormData['price'] != $price) { //前回までキャストしていたが、ゆるい判定でもいい
      //未入力チェック
      validRequired($price, 'price');
      //半角数字チェック
      validNumber($price, 'price');
  ...
  if(empty($err_msg)){
    ...
    try {
      ...
      $dbh = dbConnect();
      //SQL文作成
      //編集画面の場合はUPDATE文、新規登録画面の場合はINSERT分を生成
      if($edit_flg){
        debug('DB更新です。');
        $sql = 'UPDATE product SET name = :name, category_id = :category, price = :price, detail = :comment, pic1 = :pic1, pic2 = :pic2, pic3 = :pic3 WHERE user_id = :u_id AND id = :p_id';
        ...
        debug('DB新規登録です。');
        $sql = 'INSERT INTO product (name, category_id, price, detail, pic1, pic2, pic3, user_id, create_date) VALUES (:name, :category_id, :price, :detail, :pic1, :pic2, :pic3, :u_id, :date)';
        $data = array(':name' => $name, ':category_id' => $category, ':price' => $price, ':detail' => $comment, ':pic1' => $pic1, ':pic2' => $pic2, ':pic3' => $pic3, ':u_id' => $_SESSION['user_id'], ':date' => date('Y-m-d H:i:s'));
      }
      debug('SQL:'. $sql);
      ...
      <div class="form-container">
        <form action="" method="post" class="form" enctype="multipart/form-data" style="width:100%; box-sizing:border-box;">
          ...
          <label class="<?php if (!empty($err_msg['comment'])) echo 'err'; ?>">
            詳細
            <textarea name="comment" id="js-count" cols="30" rows="10" style="height:150px;"><?php echo getFormData('detail'); ?></textarea>
          </label>
          ...
          <div style="overflow: hidden;">
            <div class="imgDrop-container">
              画像1
              <label class="area-drop <?php if (!empty($mrr_msg['pic1'])) echo 'err'; ?>">
            ...
            <div class="imgDrop-container">
              画像2
              <label class="area-drop <?php if (!empty($mrr_msg['pic2'])) echo 'err'; ?>">
                <input type="hidden" name="MAX_FILE_SIZE" value="3145728">
            ...
            <div class="imgDrop-container">
              画像3
              <label class="area-drop <?php if (!empty($mrr_msg['pic3'])) echo 'err'; ?>">
                <input type="hidden" name="MAX_FILE_SIZE" value="3145728">
